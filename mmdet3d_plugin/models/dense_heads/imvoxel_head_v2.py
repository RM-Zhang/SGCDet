...
from mmdet3d_plugin.models.detectors.utils import get_points
...
class ImVoxelHeadV2(nn.Module):
    ...
    def _init_layers(self, n_channels, n_reg_outs):
        ...
        self.scales = nn.ModuleList([Scale(1.) for _ in range(self.n_scales)]) # learning scale for reg_conv
    ...
    def loss(self,
        ...
        loss_centerness, loss_bbox, loss_cls, sem_occ, geo_occ = [], [], [], [], []
        for i in range(len(img_metas)):
            img_loss_centerness, img_loss_bbox, img_loss_cls, per_batch_sem_occ, per_batch_geo_occ = self._loss_single(
            ...
            sem_occ.append(per_batch_sem_occ)
            geo_occ.append(per_batch_geo_occ)
        return dict(
        ...
        ), torch.stack(sem_occ), torch.stack(geo_occ)
    ...
    def _loss_single(self,
        ...
        """
        featmap_sizes = [featmap.size()[-3:] for featmap in centernesses] # [[H, W, Z], [H/2, W/2, Z/2], ...]
        ...
        centerness_targets, bbox_targets, labels, geo_occ_box = self.get_targets(mlvl_points, gt_bboxes, gt_labels)
        ...
        n_pos = max(reduce_mean(n_pos), 1.) # occupied points
        if torch.any(flatten_valids): # cls loss: all points
        ...
        if len(pos_inds) > 0: # centerness and bbox loss: occupied points
        ...
        return loss_centerness, loss_bbox, loss_cls, labels, geo_occ_box
    ...
    def _get_bboxes_single(self,
        ...
        for centerness, bbox_pred, cls_score, valid, points in zip(
            ...
            scores = cls_score.permute(1, 2, 3, 0).reshape(-1, self.n_classes).sigmoid() # sigmoid for every class
...
def compute_centerness(bbox_targets):
    x_dims = bbox_targets[..., [0, 1]] # [n_points, n_boxes, 2]
...
class ScanNetImVoxelHeadV2(ImVoxelHeadV2):
    ...
    def get_targets(self, points, gt_bboxes, gt_labels):
        ...
        ] # scale information: 0 1 2, 0-detailed
        points = torch.cat(points, dim=0).to(gt_labels.device) # [n_points, 3]
        scales = torch.cat(expanded_scales, dim=0)

        # below is based on FCOSHead._get_target_single
        n_points = len(points)
        n_boxes = len(gt_bboxes)
        volumes = gt_bboxes.volume.to(points.device) # [n_boxes]
        volumes = volumes.expand(n_points, n_boxes).contiguous() # [n_points, n_boxes]
        gt_bboxes = torch.cat((gt_bboxes.gravity_center, gt_bboxes.tensor[:, 3:6]), dim=1) # [n_boxes, 6]
        gt_bboxes = gt_bboxes.to(points.device).expand(n_points, n_boxes, 6) # [n_points, n_boxes, 6] center points and hwl
        expanded_points = points.unsqueeze(1).expand(n_points, n_boxes, 3) # [n_points, n_boxes, 3] points coordinates 
        dx_min = expanded_points[..., 0] - gt_bboxes[..., 0] + gt_bboxes[..., 3] / 2 # points - box left
        dx_max = gt_bboxes[..., 0] + gt_bboxes[..., 3] / 2 - expanded_points[..., 0] # box right - points
        ...
        bbox_targets = torch.stack((dx_min, dx_max, dy_min, dy_max, dz_min, dz_max), dim=-1) # [n_points, n_boxes, 6]

        # condition1: inside a gt bbox
        inside_gt_bbox_mask = bbox_targets[..., :6].min(-1)[0] > 0  # skip angle [n_points, n_boxes]
        ...
        # find best scale
        n_pos_points_per_scale = torch.stack(n_pos_points_per_scale, dim=0) # [n_scales, n_boxes]
        ...
        # For a bounding box target:
        # if all scales get >= 27 points in the box: use the last scale
        # else: use the smallest scale of >= 27 points
        ...
        inside_best_scale_mask = best_scale == scales # [n_points, n_boxes]

        # condition3: limit topk locations per box by centerness
        centerness = compute_centerness(bbox_targets) # [n_points, n_boxes]
        ...
        top_centerness = torch.topk(centerness, self.centerness_topk + 1, dim=0).values[-1] # [n_boxes]
        ...
        volumes = torch.where(inside_top_centerness_mask, volumes, torch.ones_like(volumes) * float_max) # [n_points, n_boxes]
        min_area, min_area_inds = volumes.min(dim=1)

        labels = gt_labels[min_area_inds] # [n_points]
        labels = torch.where(min_area == float_max, torch.ones_like(labels) * -1, labels) # [n_points], free space -1
        bbox_targets = bbox_targets[range(n_points), min_area_inds] # [n_points, 6]
        centerness_targets = compute_centerness(bbox_targets) # [n_points]

        geo_occ_box = inside_gt_bbox_mask.any(dim=1) # [n_points]
        
        return centerness_targets, self._bbox_pred_to_bbox(points, bbox_targets), labels, geo_occ_box
...
class SunRgbdImVoxelHeadV2(ImVoxelHeadV2):
    ...
    def get_targets(self, points, gt_bboxes, gt_labels):
        ...
        return centerness_targets, gt_bboxes[range(n_points), min_area_inds], labels, geo_occ_box
    ...
    def _nms(self, bboxes, scores, img_meta):
        ...
        bboxes, scores, labels = res[0], res[1], res[2]
